...
let db = {};
db.pkmn = require('../../../pokemongo-json-pokedex/output/pokemon.json');
db.move = require('../../../pokemongo-json-pokedex/output/move.json');
...
class Pokemon {
    constructor(ctx) {
        ...
        let pkmn = search(db.pkmn, 'name', ctx.name);
        this.name = pkmn.name;
        this.dex = pkmn.dex;
        this.types = [];
        ...
        if (Math.abs(this.iv.percent - Math.floor(this.iv.percent)) !== 0)
            this.iv.percent = this.iv.percent.toFixed(1);
        this.stats = {
            base: {},
            effective: {}
        };
    ...
    calcCpm(level) {
        let cpm = function (lv) {
            let a = [0.094, 0.16639787, 0.21573247, 0.25572005, 0.29024988, 0.3210876, 0.34921268, 0.3752356, 0.39956728, 0.4225, 0.44310755, 0.4627984, 0.48168495, 0.49985844, 0.51739395, 0.5343543, 0.5507927, 0.5667545, 0.5822789, 0.5974, 0.6121573, 0.6265671, 0.64065295, 0.65443563, 0.667934, 0.6811649, 0.69414365, 0.7068842, 0.7193991, 0.7317, 0.7377695, 0.74378943, 0.74976104, 0.7556855, 0.76156384, 0.76739717, 0.7731865, 0.77893275, 0.784637, 0.7903];
            return a[lv - 1];
        }
    ...
    for (const multiplier in typeMultiplers) {
        if (typeMultiplers.hasOwnProperty(multiplier)) {
            if (typeMultipliersStr.hasOwnProperty(typeMultiplers[multiplier]))
                ...
                typeMultipliersStr[typeMultiplers[multiplier]] = [multiplier];
        }
    }
...
function isPokemonName(name) {
    return search(db.pkmn, 'name', name) ? true : false;
}

function isMoveName(name) {
    return search(db.move, 'name', name) ? true : (search(db.move, 'name', name + ' Fast') ? true : false);
}
...
function search(source, key, value) {
    return source.filter((el) => {
        return el[key].compare(clean(value));
    })[0];
};
...
function typeColor(obj) {
    ...
    let hex2dec = (hex) => parseInt(hex, 16).toString(10);
...
function typeCode(obj) {
    ...
    if (obj.constructor.name.compare('Pokemon')) {
        ...
        else return [tc[obj.types[0].toLowerCase()], tc[obj.types[1].toLowerCase()]];
    } else {
...
function moveStr(obj) {
    ...
    if (obj.constructor.name.compare('Pokemon')) {
        ...
        for (let i = 0; i < obj.moves.fast.length; i++) {
        ...
        for (let i = 0; i < obj.moves.charge.length; i++) {
...
function movesetStr(obj) {
    ...
    let tempArr = [];
    for (let i = 0; i < obj.movesets.length; i++) {
...
function levelComboStr(ctx) {
    // Display all cp combos for iv% > 90 & pokemon.level
    const ivLevelGuess = function (cp, hp) {
        let stats = [];
        let percent = 0;
                ...
                for (let j = 1; j < 15; j++) {
                    for (let k = 1; k < 15; k++) {
                        ...
                        if (cp == Math.floor(pkmn.cp)) {
                            if (!hp)
                                stats.push([i + 1, j + 1, k + 1, parseFloat(percent.toFixed(1)), Math.floor(pkmn.cp), pkmn.hp, lv]);
                            else if (hp == pkmn.hp)
                                stats.push([i + 1, j + 1, k + 1, parseFloat(percent.toFixed(1)), Math.floor(pkmn.cp), pkmn.hp, lv]);
                        }
                    }
        ...
        return stats.sort((a, b) => {
            return b[3] - a[3]; // Ordered by descending cp values
        });
    }

    //console.log(ctx);
    let stats = ivLevelGuess(ctx.cp, ctx.hp);
...
function cpComboStr(pokemon) {
    ...
    // Display all cp combos for iv% > 90 & pokemon.level
    const cpGuess = function (pokemon) {
        let stats = [];
        let percent = 0;
        let pkmn = new Pokemon({
            name: pokemon.name,
        ...
        console.timeEnd('cpComboStr');
        return stats.sort((a, b) => {
            return b[4] - a[4]; // Ordered by descending cp values
        });
    }

    let stats = cpGuess(pokemon);
    let str = '';
    if (stats.length > 0) {
        ...
        for (let i = 1; i < 15; i++) {
            for (let j = 1; j < 15; j++) {
                for (let k = 1; k < 15; k++) {
                    ...
                    if (cp == Math.floor(pkmn.cp)) {
                        stats.push([i + 1, j + 1, k + 1, percent.toFixed(1), pkmn.hp]);
                    }
                }
            }
        }
