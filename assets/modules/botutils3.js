...
async function findPointInPolygon(coordinates) {
    if (coordinates) {
        try {
            //console.log('-----------------------------------------------------------------');
            //console.log(spawn);
            ...
            for (let i = 0; i < polygons.length; i++) {
                if (turf.inside(createPoint(coordinates), polygons[i])) {
...
async function getRecipients(spawn, client) {
    ...
    if (spawn.neighbourhood === 'laval') { // Push spawn alert to Laval server
        // SPAWN IS IN LAVAL
        if (!client.guilds.get('343117705974644748').roles.find('name', spawn.name.toLowerCase())) {
            ...
            await client.guilds.get('343117705974644748').createRole({
                ...
                .then(function (role) {
                    // await sendAll([role.guild.channels.find('name', 'laval_spawns')], embed);
        ...
        // await sendAll([client.guilds.get('352462877845749762').members.find('displayName', 'uphillsimplex')], embed);
        return [client.guilds.get('352462877845749762').members.find('displayName', 'uphillsimplex')];
    }
}

function createEmbed(spawn, client) {
    //const client = ctx.client;
    //const spawn = ctx.spawn;
